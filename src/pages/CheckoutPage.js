...
const StripePaymentForm = ({ finalTotal, onPaymentReady }) => {
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      if (result.paymentIntent.status === "succeeded") {
        onPaymentReady(result.paymentIntent.id);
      }
    } catch (err) {
      console.error(err);
...
const CheckoutPage = () => {
  ...
  return (
    <div className="checkout-page">
      {/* Blur only content behind modal */}
      <div className={`checkout-content ${showModal ? "blurred-bg" : ""}`}>
